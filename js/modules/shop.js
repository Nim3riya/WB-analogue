...
const cart = {
    cartGoods: JSON.parse(localStorage.getItem('cardWilb')) || [],

    updateLocalStorage() {
        localStorage.setItem('cardWilb', JSON.stringify(this.cartGoods))
    },
...
modalForm.addEventListener('submit', e => {
    ...
    if (validForm(formData) && cart.getCountCartGoods()) {
        formData.append('cart', JSON.stringify(cartTableGoods));
        postData(formData)
            .then(response => {
                if (!response.ok) {
